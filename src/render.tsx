...
export const UISourceMap = new Map([
  [
    'text',
    (item) => {
      return {
        ...
        key: item?.attr_key,
        extra: item?.attr_desc,
        placeholder: `请输入${item?.attr_name}`,
        uiType: 'input',
        rules: [
          {
            required: item?.checked,
            message: `请输入${item?.attr_name}`,
          },
        ],
      };
    },
    ...
    (item) => {
      return {
        ...
        rules: [
          {
            required: item?.checked,
            message: `请选择${item?.attr_name}`,
          },
        ],
        uploadProps: {
          action: '/api/attachment/content',
          maxFileLength: 1,
          fileSize: 1,
          stringValue: true,
        },
  ...
  [
    ...
    (item) => {
      return {
        ...
        placeholder: `请选择${item?.attr_name}`,
        options: item?.attr_param
          ? typeof item.attr_param === 'string'
            ? JSON.parse(item.attr_param)
            : item.attr_param
          : [],
        selectOptions: {
          title: 'label',
