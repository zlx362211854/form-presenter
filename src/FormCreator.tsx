...
import { Form, Divider, Row, Col, Button, Spin } from "antd";
...
interface IProps extends IFormPresenterOptions {
  componentProps: IComponentProps; // 渲染时来自组件的props
}
...
const FormCreator = forwardRef((props: IProps, ref) => {
          ...
          {fields?.map((field, index) => {
            ...
            if (typeof formItem.extra === "string") {
              ...
              const fieldInitValue =
                initFormValues[formItem.key] ||
                form.getFieldValue(formItem.key);
  ...
  return (
    <div style={{ overflowX: "hidden" }} className={props.wrapperClassName}>
      <Spin spinning={!!props.componentProps.loading}>
        <Form
          form={form}
